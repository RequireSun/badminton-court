...
export default function (bookingStatement: { [courtNo: string]: { bookings: IBookingStatement[]; price: number } }): string[] {
    ...
    for (const courtNo of courtNos) {
        ...
        for (const booking of bookingStatement[courtNo].bookings) {
            lines.push(`${booking.booking.date} ${dateNumToStr(booking.booking.startTime)}~${dateNumToStr(booking.booking.endTime)}${'Canceled' === booking.booking.status ? ' 违约金' : ''} ${booking.price} 元`);
        }
